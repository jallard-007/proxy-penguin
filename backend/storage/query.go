...
func (s *Storage) QueryPage(ctx context.Context, startRow, endRow int64) ([]model.Request, error) {
	...
	rows, err := s.conn.QueryContext(ctx, scanQuery+" WHERE r.id > ? AND r.id < ? ORDER BY r.id DESC", startRow, endRow)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Storage) MaxRowId(ctx context.Context) (int64, error) {
	rows, err := s.conn.QueryContext(ctx, "SELECT id FROM requests WHERE id = (SELECT MAX(id) FROM requests)")
	if err != nil {
		return 0, err
	}
	defer rows.Close()
