...
type Broker struct {
	mu          sync.RWMutex
	subscribers map[uint64]chan *model.RecordEvent
	nextID      uint64
}
...
func New() *Broker {
	return &Broker{
		subscribers: make(map[uint64]chan *model.RecordEvent),
	}
}
...
func (b *Broker) Subscribe() (uint64, <-chan *model.RecordEvent) {
	b.mu.Lock()
	defer b.mu.Unlock()
	id := b.nextID
	b.nextID++
	ch := make(chan *model.RecordEvent, 256)
	b.subscribers[id] = ch
	return id, ch
}
...
// Publish sends evt to every subscriber's channel, dropping the event for
// any subscriber whose buffer is full.
func (b *Broker) Publish(evt *model.RecordEvent) {
	b.mu.RLock()
	defer b.mu.RUnlock()
